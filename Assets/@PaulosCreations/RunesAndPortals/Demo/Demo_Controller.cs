...
public class Demo_Controller : MonoBehaviour
{
    ...
    [SerializeField] private PortalGate_Controller[] portalGateScripts;
    ...
    private IEnumerator DemoRoutine()
    {
        ...
        TogglePortalGates(true); // <- enciende TODOS los PortalGate
        ...
        yield return new WaitForSeconds(9f);
        TogglePortalGates(false); // <- apaga TODOS
    ...
    private void TogglePortalGates(bool on)
{
    if (portalGateScripts == null) return;
    foreach (var g in portalGateScripts)
        if (g) g.F_TogglePortalGate(on);
}
